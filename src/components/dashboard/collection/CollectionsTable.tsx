...
type Collection = Omit<PrismaCollection, 'categoryId'>;
...
const CollectionsTable = ({ collections }: CollectionsTableProps) => {
  ...
  const sortedItems = useMemo(() => {
    return [...items].sort((a: Collection, b: Collection) => {
      const first = a[sortDescriptor.column as keyof Collection] ?? (0 as number);
      const second = b[sortDescriptor.column as keyof Collection] ?? (0 as number);
  ...
  const renderCell = useCallback((user: Collection, columnKey: Key) => {
    const cellValue = user[columnKey as keyof Collection];
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between">
          <span className="text-small text-default-400">Total {collections.length} categories</span>
